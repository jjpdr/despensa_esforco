...
class ProdutoReporter extends Reporter
{
	...
	// 'CustomFieldExample' is an example that is not part of the `produto` table
	...
	*/
	static function GetCustomQuery($criteria)
	{
		$sql = "select
			...
			,`produto`.`prioridade` as Prioridade
			,`produto`.`qtd_minima` as QtdMinima
			,`produto`.`qtd_maxima` as QtdMaxima
		from `produto`";
	...
	*/
	...
	{
		$sql = "select count(1) as counter from `produto`";
